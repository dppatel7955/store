...
class SeoService
{
    ...
    private static function defaultTitle(?Product $product, ?Category $category): string
    {
        return match (true) {
            ...
            request()->routeIs('refund-policy') => 'Refund & Cancellation Policy - ' . self::STORE_NAME,
    ...
    private static function defaultDescription(?Product $product, ?Category $category): string
    {
        return match (true) {
            ...
            request()->routeIs('refund-policy') => 'Read cancellation, return, and refund rules for orders at ' . self::STORE_NAME . '.',
