...
new class extends Component
{
    ...
    {
        $query = Product::with(['brand', 'variants'])->where('is_active', true);
            ...
            @if($this->products->count() > 0)
                <div class="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-5">
                    @foreach($this->products as $prod)
                        ...
                        @endphp
                        <div wire:key="product-{{ $prod->id }}" x-data="{ showQuickAdd: false }" class="group relative bg-white border border-slate-200 rounded-xl sm:rounded-2xl overflow-hidden hover:border-indigo-500 hover:shadow-md transition duration-300 flex flex-col h-full">
                            ...
                            @if(count($prod->variants->where('is_active', true)) > 0)
                                ...
                                >
                                        ...
                                        @foreach($prod->variants->where('is_active', true) as $variant)
                                            @if($prod->variant_type === \App\Models\Product::VARIANT_TYPE_COLOR)
                                                <button @click="showQuickAdd = false; $wire.addToCart({{ $prod->id }}, {{ $variant->id }})" 
                                                        class="h-8 w-8 rounded-full border border-slate-200 flex items-center justify-center p-0.5 hover:scale-110 transition relative group/v"
                                                        title="{{ $variant->name }}"
                                                >
                                            ...
                                            @else
                                                <button @click="showQuickAdd = false; $wire.addToCart({{ $prod->id }}, {{ $variant->id }})" 
                                                        class="min-w-8 h-8 px-2 border border-slate-200 hover:border-indigo-600 hover:text-indigo-650 rounded-lg text-xs font-bold text-slate-700 transition flex items-center justify-center"
                                                >
                                    ...
                                    <button @click="showQuickAdd = false" class="text-[10px] text-slate-400 font-semibold text-center mt-1 hover:text-slate-600">Cancel</button>
                                </div>
                            @endif
                        </div>
                    @endforeach
