...
new class extends Component
{
    ...
    public function delete($id)
    {
        ...
        if ($category->products()->count() > 0) {
            session()->flash('error', "Cannot delete category '{$category->name}' because it contains products.");
            return;
        }
    ...
    >
        ...
        >
            ...
            <form wire:submit="save" class="p-6 space-y-4">
                ...
                <!-- Image URL -->
                <div>
                    <label class="block text-xs font-semibold text-slate-500 mb-1.5">Image URL</label>
                    <input 
                        type="text" 
                    ...
                    />
                    @error('image') <span class="text-[10px] text-rose-600 font-semibold">{{ $message }}</span> @enderror
                    
                    @if($image && !filter_var($image, FILTER_VALIDATE_URL) === false)
                        <div class="mt-2 text-center">
                            <span class="text-[10px] text-slate-450 block mb-1">Image Preview</span>
                            <img src="{{ $image }}" class="h-20 w-20 object-cover rounded-lg border border-slate-200 mx-auto" onerror="this.src='https://images.unsplash.com/photo-1531297484001-80022131f5a1?q=80&w=200&auto=format&fit=crop'">
                        </div>
                    @endif
                </div>
